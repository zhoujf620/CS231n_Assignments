...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx2, dw2, db2 = affine_backward(ds, scores_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            ...
            if i < self.num_layers and (self.normalization == 'batchnorm' or self.normalization == 'layernorm'):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            current_x, cache = affine_forward(
                current_x, self.params['W%d'%i], self.params['b%d'%i])
            caches['affine%d'%i] = cache

            if self.normalization == 'batchnorm':
                current_x, cache = batchnorm_forward(
                    current_x, self.params['gamma%d'%i], self.params['beta%d'%i], self.bn_params[i-1])
                caches['batchnorm%d'%i] = cache

            elif self.normalization == 'layernorm':
                current_x, cache = layernorm_forward(
                    current_x, self.params['gamma%d'%i], self.params['beta%d'%i], self.bn_params[i-1])
                caches['layernorm%d'%i] = cache
        ...
        dx, dw, db = affine_backward(ds, cache_out)
        ...
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dx = dropout_backward(dx, caches['dropout%d'%i])

            # backward pass through current ReLU layer
            dx = relu_backward(dx, caches['relu%d'%i])

            if self.normalization == 'batchnorm':
                dx, d_gamma, d_beta = batchnorm_backward(
                    dx, caches['batchnorm%d'%i])
                grads['gamma%d'%i] = d_gamma
                grads['beta%d'%i] = d_beta

            elif self.normalization == 'layernorm':
                dx, d_gamma, d_beta = layernorm_backward(
                    dx, caches['layernorm%d'%i])
                grads['gamma%d'%i] = d_gamma
                grads['beta%d'%i] = d_beta

            # backward pass through current affine layer
            dx, dw, db = affine_backward(dx, caches['affine%d'%i])
