...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        captions_emb, w2v_cache = word_embedding_forward(captions_in, W_embed)

        if self.cell_type == 'rnn':
            rnn_hidden, rnn_cache = rnn_forward(captions_emb, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            rnn_hidden, rnn_cache = lstm_forward(captions_emb, h0, Wx, Wh, b)
        else:
            raise ValueError('Invalid cell_type "%s"' % self.cell_type)

        captions_pred, vocab_cache = temporal_affine_forward(rnn_hidden, W_vocab, b_vocab)
        loss, grad_loss = temporal_softmax_loss(captions_pred, captions_out, mask, verbose=False)
        ...
        # save all gradients
        grads = {k: v for k, v in zip(
            ['W_embed', 'W_proj', 'b_proj', 'Wx', 'Wh', 'b', 'W_vocab', 'b_vocab'],
            [grad_w_embed, grad_w_proj, grad_b_proj, dWx, dWh, db, grad_w_vocab, grad_b_vocab])}
