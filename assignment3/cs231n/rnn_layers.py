...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        ...
        if t == 0:
            ...
            prev_h = h[:, t-1, :]
        step_cache = (prev_h, xt, next_h, Wx, Wh, b)
        next_h = prev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, z_i, z_f, z_o, z_g, prev_c, prev_h, next_c, Wx, Wh)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, z_i, z_f, z_o, z_g, prev_c, prev_h, next_c, Wx, Wh = cache

    dz_o = np.tanh(next_c) * dnext_h
    dnext_c += z_o * (1 - np.tanh(next_c) ** 2) * dnext_h
    dz_f = prev_c * dnext_c
    dz_i = z_g * dnext_c
    dz_g = z_i * dnext_c
    dprev_c = z_f * dnext_c
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    x, z_i, z_f, z_o, z_g, prev_c, prev_h, next_c, Wx, Wh = cache[T-1]
    D = x.shape[1]
